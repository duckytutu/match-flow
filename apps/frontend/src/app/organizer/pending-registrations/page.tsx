...
export default function OrganizerPendingRegistrations() {
  ...
  const fetchPendingRegistrations = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get(`/event-registrations/organizer/${user!.id}/pending`);
  ...
  const handleAction = (registration: EventRegistration, type: 'approve' | 'reject' | 'request-info') => {
    if (type === 'approve') {
      ...
      // Show modal for reject and request-info
  ...
  const approveRegistration = async (registration: EventRegistration) => {
    setProcessing(true);
    try {
      await apiClient.patch(`/event-registrations/${registration.id}/approve`);
      
      setToastMessage('Đăng ký đã được phê duyệt thành công!');
      ...
      const error = err as { response?: { data?: { message?: string } } };
      const errorMessage = error.response?.data?.message || 'Phê duyệt thất bại. Vui lòng thử lại.';
  ...
  const confirmAction = async () => {
    if (!selectedRegistration || actionType === 'approve') return;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          ...
          >
            ← Quay lại My Tournaments
          </Link>
        </div>

        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground">Quản lý đăng ký chờ xét duyệt</h1>
        ...
        {/* Action Modal - Only for reject and request-info */}
        {showActionModal && selectedRegistration && actionType !== 'approve' && (
          <Modal
